...
test('tracker reports the soonest due time across tracked races, or undefined when idle', () => {
  const tracker = new ActiveRaceTracker()
  assert.equal(tracker.earliestDueAt(), undefined)

  tracker.observe(0n, { status: 0, bettingEndTime: 300n })
  tracker.observe(1n, { status: 1, raceEndTime: 150n })
  assert.equal(tracker.earliestDueAt(), 150n)

  tracker.observe(1n, { status: 2 }) // resolves -> terminal -> no longer tracked
  assert.equal(tracker.earliestDueAt(), 300n)

  tracker.observe(0n, { status: 3 })
  assert.equal(tracker.earliestDueAt(), undefined)
})
