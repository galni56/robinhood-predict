...
export function AssetRaceResultView({
  ...
  const action = race.source === 'preview' ? (
    ...
    <button onClick={onClaim} disabled={!!txLabel || position?.settled} className="w-full rounded-lg bg-gradient-to-r from-[#C6FF3D] to-[#8FBF1F] py-2.5 text-sm font-black text-black disabled:opacity-40">
      {position?.settled ? 'Already claimed' : txLabel ?? `Claim ${formatUsdRaw(payout, tokenDecimals)} ${tokenLabel}`}
    </button>
  ) : refundable ? (
    <button onClick={onRefund} disabled={!!txLabel} className="w-full rounded-lg bg-white py-2.5 text-sm font-black text-black disabled:opacity-40">
  ...
  return (
    <div className="space-y-5">
      <div className={`relative overflow-hidden rounded-2xl border p-5 ${resolved ? meme ? 'race-result-enter border-orange-300/40 bg-gradient-to-br from-fuchsia-500/[0.18] via-[#281633] to-orange-400/[0.12]' : 'race-result-enter border-[#C6FF3D]/30 bg-[#11170e]/90' : 'border-amber-400/25 bg-amber-400/5'}`}>
      ...
      {position?.exists && (
        <div className={`grid grid-cols-2 gap-3 rounded-xl border p-4 transition-all sm:grid-cols-4 ${lost ? 'border-white/5 bg-[#12121c]/75 opacity-80' : won ? meme ? 'border-orange-300/30 bg-fuchsia-400/[0.08]' : 'border-[#C6FF3D]/25 bg-[#12121c]/95' : 'border-white/10 bg-[#12121c]/95'}`}>
          ...
          <div><div className="text-xs text-white/35">Result</div><div className={won ? 'font-bold text-[#C6FF3D]' : resolved ? 'font-bold text-rose-400' : 'font-bold text-amber-300'}>{won ? 'WON' : resolved ? 'LOST' : 'REFUND'}</div></div>
      ...
      {resolved && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-[#12121c]/95 p-4"><div className="text-xs text-white/35">Winning pool</div><div className="mt-1 font-mono font-bold">{formatUsdRaw(race.winningPool, tokenDecimals)} {tokenLabel}</div></div>
          <div className="rounded-xl border border-white/10 bg-[#12121c]/95 p-4"><div className="text-xs text-white/35">Total pool</div><div className="mt-1 font-mono font-bold">{formatUsdRaw(race.totalPool, tokenDecimals)} {tokenLabel}</div></div>
          <div className="col-span-2 rounded-xl border border-white/10 bg-[#12121c]/95 p-4 sm:col-span-1"><div className="text-xs text-white/35">Fee from losing pool</div><div className="mt-1 font-mono font-bold">{formatUsdRaw(race.protocolFee, tokenDecimals)} {tokenLabel}</div></div>
        </div>
      )}
      ...
      <div>
        ...
        {resolved || voided ? <AssetRaceLeaderboard race={race} position={position} final /> : (
          <div className="space-y-2">
            {race.assets.map((asset) => <div key={asset.assetIndex} className="flex justify-between rounded-lg border border-white/10 bg-[#12121c]/95 px-3 py-2 text-sm"><span>{asset.symbol}</span><span className="font-mono text-white/45">{formatUsdRaw(asset.pool, tokenDecimals)} {tokenLabel} backed</span></div>)}
          </div>
        )}
      </div>
