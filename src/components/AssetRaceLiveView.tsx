...
export function AssetRaceLiveView({ race, position, nowMs, tokenDecimals, tokenLabel }: { race: AssetRaceViewModel; position?: AssetRacePosition; nowMs: number; tokenDecimals: number; tokenLabel: string }) {
  ...
  return (
    <div className="space-y-5">
      <div className={`relative overflow-hidden rounded-2xl border p-5 ${meme ? 'border-fuchsia-300/30 bg-gradient-to-br from-[#29143c]/95 to-[#351713]/90' : 'border-[#C6FF3D]/30 bg-[#11170e]/90'}`}>
        <div className={`absolute -right-12 -top-20 h-52 w-52 rounded-full blur-3xl ${meme ? 'bg-orange-400/25' : 'bg-[#C6FF3D]/10'}`} />
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            <div className={`mb-2 flex items-center gap-2 text-xs font-black tracking-[0.25em] ${meme ? 'text-orange-200' : 'text-[#C6FF3D]'}`}>
      ...
      {position?.exists && selectedAsset && (
        <div className="grid grid-cols-2 gap-3 rounded-xl border border-white/10 bg-[#12121c]/95 p-4 text-sm sm:grid-cols-3">
          <div><div className="text-xs text-white/35">Your asset</div><div className="font-bold text-[#C6FF3D]">{selectedAsset.symbol}</div></div>
