  ...
  return (
    <div className={`mx-auto max-w-4xl px-4 py-8 ${mode === 'memes' ? 'asset-race-meme' : ''}`}>
      ...
      <div className="mt-5 grid grid-cols-2 rounded-2xl border border-white/10 bg-black/30 p-1.5">
        {(['stocks', 'memes'] as const).map((item) => <button key={item} type="button" onClick={() => selectMode(item)} className={`rounded-xl px-5 py-3 text-sm font-black tracking-[0.16em] transition-all ${mode === item ? item === 'memes' ? 'bg-gradient-to-r from-fuchsia-500 to-orange-400 text-white' : 'bg-[#C6FF3D] text-black' : 'text-white/40 hover:text-white'}`}>{item === 'stocks' ? '📈 STOCKS' : '🚀 MEMES'}</button>)}
      </div>

      <div className={`mt-5 rounded-2xl border p-5 sm:p-7 ${mode === 'memes' ? 'border-fuchsia-300/25 bg-gradient-to-br from-[#2a153b]/95 to-[#321715]/90' : 'border-[#C6FF3D]/25 bg-[#11170e]/90'}`}>
        <div className={`text-xs font-black tracking-[0.22em] ${mode === 'memes' ? 'text-orange-200' : 'text-[#C6FF3D]'}`}>CREATE COMMUNITY {mode === 'memes' ? 'MEME' : 'STOCK'} RACE</div>
        ...
        {isLocalAssetRace && <p className="mt-3 text-xs font-bold text-[#C6FF3D]">LOCAL TEST NETWORK · NO REAL FUNDS</p>}
      </div>
      ...
      {!ASSET_RACE_ADDRESS ? (
        ...
        <div className="mt-5 space-y-5 rounded-2xl border border-white/10 bg-[#12121c]/95 p-5 sm:p-7">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-black/25 px-3 py-2.5 outline-none transition-colors focus:border-[#C6FF3D]/50"
            />
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-white/10 bg-[#0d0d15] px-3 py-2.5 outline-none focus:border-[#C6FF3D]/50"
            >
          ...
          <div>
            <div className="mb-2 flex items-end justify-between gap-3">
              ...
              <span className="shrink-0 whitespace-nowrap font-mono text-sm font-bold text-[#C6FF3D]">{selected.length} / 6</span>
            </div>
            {selected.length > 0 && (
              <div className="mb-3 flex flex-wrap gap-2">
                {selected.map((asset) => (
                  <button key={asset.assetId} type="button" onClick={() => toggleAsset(asset)} className="rounded-full border border-[#C6FF3D]/25 bg-[#C6FF3D]/10 px-3 py-1 text-xs font-bold text-[#C6FF3D]">
                    {asset.symbol} ×
                  </button>
                ))}
              </div>
          ...
          {!isConnected ? <WalletOptionsList /> : !onRightChain ? (
            <button onClick={() => switchChain({ chainId: assetRaceChain.id })} disabled={isSwitching} className="w-full rounded-lg bg-amber-400 py-2.5 text-sm font-black text-black disabled:opacity-50">
