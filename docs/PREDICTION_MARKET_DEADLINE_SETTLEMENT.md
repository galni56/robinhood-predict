...
Status: implemented and tested locally on 2026-09-23; **not deployed**. The
configured mainnet address is still the legacy contract whose result depends on
the price available when `resolve()` executes.
...
`PREDICTION_MARKET_RPC_URL` is intentionally mandatory and does not fall back
to the Asset Race RPC: production PredictionMarket traffic uses a separately
provisioned endpoint. `PREDICTION_MARKET_POOL_RPC_URL` may be omitted when that
same endpoint supports archive reads. The corresponding `ASSET_RACE_*` oracle,
...
1. Simulate deployment with the existing USDG and SignedPoolRaceOracle.
...
5. Preserve a frontend build pointing to the legacy contract until all funded
   legacy markets have a refund/claim path.
6. Only after explicit approval enable keeper writes and switch the frontend.

A read-only check on 2026-09-23 found a funded legacy market at the old address.
Do not hide that address without retaining its refund path. Frontend rollback is
restoring the previous build/address; a deployed contract cannot be rolled back.
